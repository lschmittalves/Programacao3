...
public class LabirintoFactory {
    ...
    public Modelos.Labirinto gerarLabirinto(List<String> labirintoTexto) throws RuntimeException {
        ...
        for (int i = 0; i < labirintoTexto.size(); i++) {
            ...
            for (int j = 0; j < labirintoTexto.get(i).length(); j++) {
                ...
                switch (atual) {
                    case '.':
                        linha.put(j, new Obstaculo(j, i, EnumTipoObstaculo.ESPACO_BRANCO));
                        break;
                    case '#':
                    ...
                    case 'C':
                        ...
                        break;
                    case 'T':
                        linha.put(j, new Obstaculo(j, i, EnumTipoObstaculo.ARMADILHA));
                        break;
                    default:
                        throw new RuntimeException("Caracter inválido!");
                }
        ...
        if (inicio == null) {
            throw new RuntimeException("Labirinto não possui início.");
        }
